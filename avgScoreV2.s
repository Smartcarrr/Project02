...
sumStart:
    ...
    add $t1, $a0, $t1        # Address of current element
    lw $t2, 0($t1)           # Load element from array into $t3
    sw $t2, 0($sp)           #Save a[i] to stack

    addi $t0, $t0, 1         # Increment index (i = i + 1)
    jal sumStart               # Continue recursion
    lw $t3, 0($sp)		#Load a[i] from the stack
    add $v0, $v0, $t3		# Add the current element to the sum
